...
export default function ScrapbookClient({ tripId }) {
  ...
  useEffect(() => {
    ...
    setTrip(trips.find((t) => t.id === resolvedTripId) || null);
  }, [resolvedTripId]);
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-5xl flex-col items-start gap-4 p-8">
      ...
      <Phone>
        <TopBar left={<Link href={resolvedTripId ? `/trips/${resolvedTripId}` : "/trips"}>Back</Link>} title="Scrapbook" />

        <Row>
          {!trip || !scrapbook ? (
            <div className="rounded-2xl border border-white/50 bg-white/60 p-4 text-sm text-slate-700 shadow-sm backdrop-blur">
              No scrapbook yet.
              <div className="mt-2 text-xs text-slate-600">Build a trip with concierge, then mark it completed.</div>
              <div className="mt-3">
                <CTA>
            ...
            <div className="grid gap-3">
              ...
              <div className="rounded-2xl border border-white/50 bg-white/60 p-4 shadow-sm backdrop-blur">
                ...
                <div className="mt-3 grid gap-2">
                  {scrapbook.seenAndMissed.seen.slice(0, 4).map((m) => (
                    <div key={m.id} className="rounded-xl border border-white/55 bg-white/70 px-3 py-2 text-xs text-slate-700">
              ...
              <div className="rounded-2xl border border-white/50 bg-white/60 p-4 shadow-sm backdrop-blur">
                ...
                <div className="mt-1 text-sm text-slate-700">A shareable moment—UI only in the demo.</div>
                <div className="mt-3">
                  <CTA>
