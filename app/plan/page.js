...
export default function PlanTripPage() {
  return (
    <main className="mx-auto flex min-h-screen max-w-5xl flex-col items-start gap-4 p-8">
      ...
      <Phone>
        <TopBar left={<Link href="/home">Back</Link>} title="Plan a Trip" />
        ...
        <Row title="Route option">
          <Pill>Scenic route (recommended)</Pill>
          <Pill>Fastest route</Pill>
        </Row>

        <Row title="Stops">
          <div className="text-sm text-slate-700">
            Include Percy&apos;s stops (on)
          </div>
          <div className="mt-1 text-xs text-slate-500">
            We&apos;ll highlight Percy&apos;s Places that fit your charging + timing preferences.
          </div>
        </Row>

        <CTA>
          <Link href="/plan/map">Find route</Link>
        </CTA>
